...
try {
  ...
  logger.warn('Redisモジュールが見つかりません。メモリベースのフォールバックを使用します。');
}
...
const rateLimitHelpers = {
  ...
  slidingWindow: async (key, windowMs, maxRequests) => {
    const client = await getRedisClient();
    if (!client) return { isAllowed: true, currentCount: 0, remainingRequests: maxRequests };
  ...
  tokenBucket: async (key, capacity, refillRate, tokensRequested = 1) => {
    const client = await getRedisClient();
    if (!client) return { isAllowed: true, remainingTokens: capacity };
