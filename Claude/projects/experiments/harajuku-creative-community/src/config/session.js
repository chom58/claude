...
// connect-redisの動的インポート
let RedisStore;
try {
  RedisStore = require('connect-redis').default;
} catch (error) {
  logger.warn('connect-redisモジュールが見つかりません。メモリベースのセッションストアを使用します。');
}
  ...
  try {
    ...
    if (isRedisHealthy && RedisStore) {
      ...
      if (redisClient) {
        ...
        logger.warn('Redisクライアントが利用できません。メモリセッションストアを使用します');
        store = undefined;
      }
    } else {
      // メモリセッションストア（開発環境のみ）
